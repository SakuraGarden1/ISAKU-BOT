...
function gameEmbed(title, fields, footer) {
  const e = new EmbedBuilder().setColor(0xE8B84B).setTitle(`🃏 ${title}`).addFields(fields);
...
async function playRound(message, userId, action) {
  ...
  if (action === 'hit') {
    ...
    setupCollector(msg, message, userId);
    return;
  }
  ...
  if (dv > 21 || pv > dv) {
    user.cash += game.bet * 2;
    resultText = '🏆 Таны хожлоо!'; moneyText = `+₮${shortNum(game.bet)}`;
  } else if (pv === dv) {
    user.cash += game.bet;
    resultText = '🤝 Тэнцлээ'; moneyText = 'Мөнгө буцлаа';
  } else {
    resultText = '😔 Dealer хожлоо'; moneyText = `-₮${shortNum(game.bet)}`;
  }
  saveUser(userId, user);
...
module.exports = {
  ...
  async execute(message, args) {
    ...
    await addReactions(botMsg);
    setupCollector(botMsg, message, userId);
    return botMsg;
  },
};
