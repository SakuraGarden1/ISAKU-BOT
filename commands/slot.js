...
function spin(lucky = false) {
  return [0, 1, 2].map(() => {
    if (lucky && Math.random() < 0.15) {
      // Lucky: өндөр үнэт символ илүү гарах магадлал
      return SYMBOLS[Math.floor(Math.random() * 3) + 3];
    }
...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (inv.lucky_charm_active > 0) {
      luckyActive = true;
      inv.lucky_charm_active--;
      user.inventory = inv;
    }
    ...
    if (multiplier > 0) {
      ...
      embed.addFields(
        ...
        { name: '💰 Хожлоо', value: `+₮${shortNum(gross)}`, inline: true },
